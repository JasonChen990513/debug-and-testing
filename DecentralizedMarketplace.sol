...
contract DecentralizedMarketplace{
    IERC20 public ercToken = IERC20(0x3cc603c0e3C897597ef4eFBf53B8fDd0ea900917); //need to change
    uint256 a = 1 ;
    ...
    event GoodAdded(string name, string description, uint256 price, address owner, bool sellStatus);
    ...
    function addComment(string memory _name, string memory _comment, uint8 _star, uint256 goodID) public {
        bool buyBefore;
        ...
        require(buyBefore || (msg.sender == owner), "you need to buy the good first");
    ...
    function buyGood(uint256 _goodID, uint256 _amount) public  {
        //check the balance
        require(ercToken.balanceOf(msg.sender) >= _amount*goods[_goodID].price,"balance not enough buy step" );
        //check the good is avaliable
        require(goods[_goodID].sellStatus == true, "this good is unavaliable now");
        require( goods[_goodID].amount - _amount >= 0, string(abi.encodePacked(" Only remain ", goods[_goodID].amount)));
        //transfer token to seller
        ercToken.transferFrom(msg.sender, goods[_goodID].owner, _amount*goods[_goodID].price);
    ...
    function getBuyTotalPrice(uint256[] memory items) public view returns (uint256){
        uint256 totalAmount; 
        for ( uint i=0 ; i < carts[msg.sender].length; i++) 
        {
            ...
            {       
                if(carts[msg.sender][i].goodId == items[j]){
                    totalAmount += carts[msg.sender][i].amount*goods[carts[msg.sender][i].goodId].price;
                }
            }
        }
        return totalAmount;
    ...
    function cartCheckOut(uint256[] memory items) public {
        ...
        require(ercToken.balanceOf(msg.sender) >= totalAmount,"balance not enough cartcheckout" );
    ...
    function setTestData() public {
        ...
        addGood("apple", "this is a apple", 1 ether, 10, "https://www.collinsdictionary.com/images/full/apple_158989157.jpg", "Food");
        addGood("iphone", "this is iphone 15", 2 ether, 10, "https://shop.switch.com.my/cdn/shop/files/iPhone_15_Pink_PDP_Image_Position-1__GBEN_7cf60425-0d5a-4bc9-bfd9-645b9c86e68e.jpg?v=1717694179", "Electronics");
        addGood("bed", "this is a bed", 3 ether, 10, "https://www.fortytwo.my/media/catalog/product/cache/1/image/3000x/9df78eab33525d08d6e5fb8d27136e95/s/a/santiago_upholstered_queen_bed_grey___mg_5495__cropped.jpg", "Furniture");
        addToCart(0, 1);
        addToCart(1, 2);
        addToCart(2, 3);
    ...
    // Function to get goods by page
    // function getGoodsByPage(uint256 pageNumber) public view returns (Good[] memory) {
    //     uint256 itemsPerPage = 10;
    //     uint256 start = (pageNumber - 1)  * itemsPerPage;
    //     uint256 end = start + itemsPerPage;
        
    //     require(start < goods.length, "Page number out of range");

    //     if (end > goods.length) {
    //         end = goods.length;
    //     }

    //     Good[] memory pageGoods = new Good[](end - start);
    //     for (uint256 i = start; i < end; i++) {
    //         pageGoods[i - start] = goods[i];
    //     }
